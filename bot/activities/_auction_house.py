"""
_auction_house.py — Black Market's Auction House mode (port of C#
...
class _AuctionHouse:
    ...
    def run(self):
        ...
        while True:
            ...
            action, pos = self._find_first(screen, targets)
            if action is None:
                go_home(bot, screen)
            else:
                self._handle(action, pos, screen)

    def _find_first(self, screen, targets):
        for path, action in targets:
            # A lot's bid row is measured from the top of its VIP tag.
            pos = self.bot.find(path, screen=screen, center=action != LOT)
            if pos is not None:
                return action, pos
        return None, None
